...
void OLED_GPIO_Init(void)
{
   ...
   RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;  //不亮改PP
...
void OLED_DrawSine(void)//uint8_t freq,uint8_t vcc
{
	for(uint8_t x=0;x<96;x++)
{ //(O,44) (96,44)
	//1000hz 对应两个波形(float)(96/(1000/2))        2000hz对应四个波形
	//3.3v
	
	
	
	OLED_DrawPoint(x+8,-sin(x/48.0*2*3.14)*12+12+40 );
	
//	OLED_DrawLine(8,52,113,52);   //X轴
//	OLED_DrawLine(108,48,113,52); OLED_DrawLine(108,56,113,52);  //X轴箭头
//	OLED_ShowString(113,37,"t",OLED_8X16);   //X轴单位
//	
//  OLED_DrawLine(8,25,8,52);     //Y轴
//	OLED_DrawLine(8,25,4,30); OLED_DrawLine(8,25,12,30);                         //Y轴箭头
//	OLED_ShowString(14,25,"V",OLED_6X8);    //Y轴单位
//	
...
void OLED_DrawTriangleWave(void)
{

		OLED_DrawLine(8,52,20,40);
		OLED_DrawLine(44,64,20,40);
		OLED_DrawLine(44,64,68,40);
		OLED_DrawLine(92,64,68,40);
		OLED_DrawLine(92,64,104,52);

}	

void OLED_DrawSquarewave(uint16_t setpwm)
{	
	
	  uint8_t x=18+(setpwm/100-10);
	
	
	  OLED_ClearArea(8,32,26,20);
	  OLED_DrawLine(8, 32, 8, 52);    //方波左边
	  OLED_DrawLine(x,32,8,32);        //方波上方
		OLED_DrawLine(x,32,x,52);    //方波右边

	  OLED_ClearArea(56,32,74,20);
	  OLED_DrawLine(56, 32, 56, 52); //方波左边
	  OLED_DrawLine(x+48,32,56,32);      //方波上方
		OLED_DrawLine(x+48,32,x+48,52);   //方波右边
	  OLED_Update();
}
